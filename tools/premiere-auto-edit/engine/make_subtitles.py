...
흐름:
  ...
  2) 정리된 오디오(없으면 원본)를 openai-whisper로 단어 단위 받아쓰기
...
MODEL      = "turbo"   # openai-whisper large-v3-turbo (로컬). tiny/base/small/medium/large-v3/turbo
...
# openai-whisper 모델 캐시 (같은 프로세스에서 재로딩 방지)
_WHISPER_MODELS = {}

# 예전 mlx HF repo 이름 → openai-whisper 숏네임
_MLX_ALIASES = {
...
def resolve_model_name(model):
    """mlx HF repo / 별칭을 openai-whisper 모델명으로 정규화."""
    if not model:
        return MODEL
    if model in _MLX_ALIASES:
        return _MLX_ALIASES[model]
    name = model.split("/")[-1]
...
def transcribe(audio, model=MODEL, initial_prompt=None, condition=False):
    """로컬 openai-whisper로 단어 단위 전사. 클라우드 API 호출 없음."""
    import whisper
    import torch

    name = resolve_model_name(model)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    use_fp16 = device == "cuda"  # CPU에선 fp16 미지원
    print(f"> 받아쓰기 중... (모델 {name}, 로컬/{device})")

    if name not in _WHISPER_MODELS:
        _WHISPER_MODELS[name] = whisper.load_model(name, device=device)
    m = _WHISPER_MODELS[name]
